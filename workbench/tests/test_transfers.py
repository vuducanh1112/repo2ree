"""Chunked workbench copy-in reassembly on the workbench side."""
...
def test_deliver_reassembles_chunks_and_cleans_up() -> None:
    store = TransferStore()
    transfer_id = store.open("/ree/dest.bin")
    ...
    def sink(source_path: str, workbench_path: str) -> None:
...
def test_deliver_removes_temp_file_even_when_sink_raises() -> None:
    ...
    def failing_sink(source_path: str, workbench_path: str) -> None:
...
def test_abort_and_abort_all_discard_partial_files() -> None:
    store = TransferStore()
    aborted = store.open("/ree/a.bin")
    store.write(aborted, 0, b"partial")
    dangling = store.open("/ree/b.bin")
