"""Workbench-side reassembly of a chunked byte transfer (the copy-in path).

The control plane streams a file to the workbench in bounded chunks (see the
wire notes in ``repo2ree_protocol.workbench``). Each open transfer is a temp
file on the serving host that chunks append to; on ``deliver`` the finished
file is handed to a sink (the backend's ``copy_in``) and deleted.
...
class TransferStore:
    """The transfers currently open on one workbench connection."""

    def __init__(self) -> None:
        self._transfers: dict[str, _Transfer] = {}
        self._lock = threading.Lock()

    def open(self, workbench_path: str) -> str:
    ...
    def deliver(self, transfer_id: str, sink: Callable[[str, str], None]) -> None:
        ...
        try:
            sink(transfer.path, transfer.workbench_path)
        finally:
