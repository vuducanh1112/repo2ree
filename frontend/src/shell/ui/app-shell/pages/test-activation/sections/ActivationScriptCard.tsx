...
import { ACTIVATION_BASE_HINTS } from "./activationScriptBaseHints";
import {
  ActivationBaseChip,
  ActivationModeSegmentedControl,
  ActivationProvenanceChip,
} from "./activationScriptUiPrimitives";

interface ActivationScriptCardProps {
  scriptPath: string;
  ...
  onClear: () => void;
  onSourceChange: (source: ActivationScriptSource | null) => void;
}

export function ActivationScriptCard(props: ActivationScriptCardProps) {
  const { scriptPath, source } = props;
  const [mode, setMode] = useState<ActivationScriptMode>(modeForActivationSource(source));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      >
        <ActivationModeSegmentedControl mode={mode} onChange={setMode} />
        <ActivationProvenanceChip source={scriptPath ? source : null} />
      </div>
...
function PickPane({
  ...
  return (
    <div style={lgContentCard(0)}>
      <div style={{ ...lgStyles.label, marginBottom: 8 }}>Existing activation script</div>
      <FilePicker
        value={scriptPath}
        onChange={handle}
        files={files}
...
function WritePane({
  ...
  const templates = defaultScriptTemplates("validate", runtimeHint);
  const [editorPath, setEditorPath] = useState(scriptPath || "activation_test.sh");
  ...
  const handleSave = () => {
    const filename = editorPath.trim() || "activation_test.sh";
    onCommit(filename, editorContent);
    onSourceChange(activationSourceAfterSave(source));
  };
...
function GeneratePane({
  ...
  const handleGenerate = () => {
    ...
    onSourceChange(activationSourceAfterGenerate(template.key));
  };
  ...
  return (
    <div style={lgContentCard(0)}>
      ...
      <div style={{ ...lgStyles.helper, marginBottom: 12 }}>
        Pick the activation style — the generator will scaffold a smoke-test script you can run
        as-is or tweak in <strong>Write</strong>.
      </div>
      <div
        style={{
          display: "grid",
      ...
      >
        {templates.map((template) => (
          <ActivationBaseChip
            ...
            hint={ACTIVATION_BASE_HINTS[template.key]}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hasScript ? (
          <>
            Activation script: <code>{scriptPath}</code>
          </>
        ) : (
          "No activation script selected yet."
        )}
      </span>
      {hasScript && (
        <button
