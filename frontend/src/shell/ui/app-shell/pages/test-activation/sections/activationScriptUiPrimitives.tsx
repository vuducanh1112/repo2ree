...
export function ActivationProvenanceChip({ source }: { source: ActivationScriptSource | null }) {
  return <span style={lgPillChip(!!source)}>{activationProvenanceLabel(source)}</span>;
}

export function ActivationModeSegmentedControl({
  mode,
  onChange,
}: {
  mode: ActivationScriptMode;
  onChange: (next: ActivationScriptMode) => void;
}) {
  const options: Array<{ key: ActivationScriptMode; label: string; icon: React.ReactNode }> = [
...
export function ActivationBaseChip({
  active,
  label,
  hint,
  onClick,
