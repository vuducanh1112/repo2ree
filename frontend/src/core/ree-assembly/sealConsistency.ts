...
// Consistency steps → assembly operation keys (which double as canvas node
// page keys; see PAGE in the shell). Every stale experiment ambers the one
// Experiments node.
const STEP_TO_ASSEMBLY_KEY: Record<string, string> = {
  build_runtime: "build",
  generate_sbom: "sbom",
  activation_test: "activation",
};

/** Assembly-step keys whose recorded result is stale, for badge amber-ing. */
export function staleAssemblyStepKeys(report: ConsistencyReport | undefined): ReadonlySet<string> {
  ...
  for (const step of report?.steps ?? []) {
    ...
    if (step.step.startsWith(EXPERIMENT_STEP_PREFIX)) {
      ...
      const key = STEP_TO_ASSEMBLY_KEY[step.step];
      if (key) keys.add(key);
    }
  }
  return keys;
