...
import { type ConsistencyReport, staleSealItems, staleStepKeys } from "./sealConsistency";
...
describe("staleStepKeys", () => {
  it("maps stale steps to step keys, folding experiments into one node", () => {
    expect(staleStepKeys(report)).toEqual(new Set(["build", "experiments"]));
  });

  it("ignores fresh and missing steps", () => {
    expect(staleStepKeys({ steps: [{ step: "generate_sbom", status: "missing" }] })).toEqual(
      new Set(),
    );
    expect(staleStepKeys(undefined)).toEqual(new Set());
  });
});
