...
import { applyExplorerAction, createInitialExplorerState } from "./explorerState";
...
describe("explorerState", () => {
  it("applies source patch outcomes to ree and source status metadata", () => {
    const initial = createInitialExplorerState(buildRee());

    const next = applyExplorerAction(initial, {
      type: "explorer/applySourcePatchOutcome",
      outcome: {
        reePatch: {
  ...
  it("records completion metadata for completed service runs", () => {
    ...
    const next = applyExplorerAction(initial, {
      type: "explorer/completeServiceRun",
      completion: {
        key: "build",
  ...
  it("resets workflow-dependent explorer state on source change", () => {
    const initial = {
      ...createInitialExplorerState(buildRee()),
    ...
    const next = applyExplorerAction(initial, {
      type: "explorer/resetWorkflowOnSourceChange",
