...
} from "../../ui/workspace-shell/providers/WorkspaceShellProvider";
import { workspaceShellSelectors } from "./WorkspaceShellSelectors";
...
describe("workspaceShellState", () => {
  ...
  it("records completion metadata for completed workflow runs", () => {
    ...
    const next = workspaceShellReducer(initial, {
      type: "workspaceShell/completeWorkflowRun",
      completion: {
        key: "build",
  ...
  it("resets workflow-dependent workspace state on source change", () => {
    ...
    const next = workspaceShellReducer(initial, {
      type: "workspaceShell/resetWorkflowOnSourceChange",
