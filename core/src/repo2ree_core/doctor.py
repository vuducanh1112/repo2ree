...
docker substrate, which handler tools are present). The workbench runs it right
...
_PROBED_TOOLS = ("syft", "git", "curl", "unzip", "tar", "gzip", "shellcheck")

# dockerd may still be starting when the probe runs (the workbench probes right
...
def run_doctor(ree_path: Path | None = None, docker_wait_seconds: float = _DOCKER_WAIT_SECONDS) -> dict[str, Any]:
    ree_path = ree_path or Path(os.environ.get("REPO2REE_WORKBENCH_ROOT", "/ree"))
...
def _probe_docker(wait_seconds: float) -> dict[str, Any]:
    ...
    while True:
        ...
        # which the workbench would read as a bench-contract violation and fail
