...
def parse_packages_file(packages_file_str: str) -> APTPackages:
    ...
    with io.StringIO(packages_file_str) as packages_file:
        for package_record in Packages.iter_paragraphs(packages_file):
            ...
            apt_packages[package_name] = APTPackageEntry(
                **{key.lower(): value for key, value in package_record.items()}
            )
...
def get_put_snapshot_sources_shell_command(
    ...
    match os_release_id:
        case OSReleaseID.DEBIAN:
            ...
            commands: list[str] = []
            commands.append(
                """if [ -e /etc/apt/sources.list.d/debian.sources ]; then rm -f /etc/apt/sources.list.d/debian.sources; fi;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}debian/{snapshot} {version_code} main" >/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}debian-security/{snapshot} {version_code}-security main" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}debian/{snapshot} {version_code}-updates main" >>/etc/apt/sources.list;"""
            )
            if debian_backports:
                commands.append(
                    f"""echo "deb [check-valid-until=no] {snapshot_archive_base}debian/{snapshot} {version_code}-backports main" >>/etc/apt/sources.list;"""
                )
        ...
        case OSReleaseID.UBUNTU:
            ...
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code} main restricted" >/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code}-updates main restricted" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code} universe" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code}-updates universe" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code} multiverse" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code}-updates multiverse" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code}-backports main restricted universe multiverse" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code}-security main restricted" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code}-security universe" >>/etc/apt/sources.list;"""
            )
            commands.append(
                f"""echo "deb [check-valid-until=no] {snapshot_archive_base}ubuntu/{snapshot} {version_code}-security multiverse" >>/etc/apt/sources.list;"""
            )
