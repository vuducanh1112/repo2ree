...
def pin_dockerfile_package_install_commands(
    ...
    for dockerfile_instruction in dockerfile_instructions:
        ...
        if dockerfile_instruction.instruction == "FROM":
            new_dockerfile_instructions.append(
...
def lookup_package_version(
    ...
    os_release_info: OSReleaseInfo,
    architecture: str,
    install_command: PackageInstallCommand,
) -> str:
    version = ""
...
def extract_packages_from_install_command(
    command_tokens: list[str], install_command: PackageInstallCommand
) -> list[str]:
    packages = []
...
def put_pinned_versions_into_install_command(
    command_tokens: list[str],
