...
def test_a_failed_write_leaves_the_previous_version_intact(tmp_path):
    ...
    with patch("repo2ree_core.persistence.files.os.replace", side_effect=OSError("boom")), pytest.raises(OSError):
...
def test_cancellation_mid_write_leaves_no_temporary_behind(tmp_path):
    ...
    with (
        patch("repo2ree_core.persistence.files.os.replace", side_effect=KeyboardInterrupt),
...
def test_concurrent_writers_do_not_share_a_temporary(tmp_path):
    ...
    with patch("repo2ree_core.persistence.files.os.replace", side_effect=capture):
