...
def _build_receipt(*, digest: str | None = _RUNTIME_DIGEST) -> BuildRuntimeReceipt:
    return BuildRuntimeReceipt(
        ...
        status="succeeded",
        produced_runtime_digest=Digest(digest) if digest else None,
    )
...
def _sbom_receipt(*, declared: str | None = _RUNTIME_DIGEST) -> GenerateSbomReceipt:
    return GenerateSbomReceipt(
        run_id=RunId("sbom-1"),
        started_at=parse_utc_instant("2026-01-01T01:00:00Z"),
        finished_at=parse_utc_instant("2026-01-01T01:00:00Z"),
        duration_ms=0,
        recorded_at=parse_utc_instant("2026-01-01T01:00:00Z"),
        status="succeeded",
        runtime_path=WorkspacePath("ree/runtime.tar"),
        declared_runtime_digest=Digest(declared) if declared else None,
        sbom_digest=Digest("sha256:" + "d" * 64),
    )
...
def _crosscheck_receipt(*, sbom_digest: str | None = _SBOM_DIGEST) -> CrossCheckSbomReceipt:
    return CrossCheckSbomReceipt(
        ...
        started_at=parse_utc_instant("2026-01-01T01:30:00Z"),
        finished_at=parse_utc_instant("2026-01-01T01:30:00Z"),
        duration_ms=0,
        recorded_at=parse_utc_instant("2026-01-01T01:30:00Z"),
        status="succeeded",
        sbom_digest=Digest(sbom_digest) if sbom_digest else None,
...
def _activation_receipt(*, declared: str | None = _RUNTIME_DIGEST) -> ActivationTestReceipt:
    return ActivationTestReceipt(
        run_id=RunId("act-1"),
        started_at=parse_utc_instant("2026-01-01T02:00:00Z"),
        finished_at=parse_utc_instant("2026-01-01T02:00:00Z"),
        duration_ms=0,
        recorded_at=parse_utc_instant("2026-01-01T02:00:00Z"),
        status="succeeded",
        run_script_path=ScriptPath("ree-scripts/activation.sh"),
        declared_runtime_digest=Digest(declared) if declared else None,
    )
...
def _experiment_receipt(
    ...
    *,
    ...
    return RunExperimentReceipt(
        run_id=RunId(f"exp-{name}"),
        started_at=parse_utc_instant("2026-01-01T03:00:00Z"),
        finished_at=parse_utc_instant("2026-01-01T03:00:00Z"),
        duration_ms=0,
        recorded_at=parse_utc_instant("2026-01-01T03:00:00Z"),
        status=status,  # type: ignore[arg-type]
        experiment_name=name,
        run_script_path=ScriptPath("ree-scripts/experiment.sh"),
        declared_runtime_digest=Digest(declared) if declared else None,
