...
from repo2ree_core.operations.read_models.ree_document import get_ree_document
...
def test_bundle_archive_honors_inclusion_flags_and_manifest_remap(tmp_path):
    ...
    archive_bytes = build_ree_archive(storage_root, ree_id)
...
def test_bundle_seals_all_results_when_results_included(tmp_path):
    ...
    with zipfile.ZipFile(io.BytesIO(build_ree_archive(storage_root, ree_id))) as zf:
        names = zf.namelist()
...
def test_build_archive_raises_when_the_sealed_archive_is_missing(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="re-seal"):
        build_ree_archive(storage_root, ree_id)


def test_get_ree_document_tags_overlay_files_as_generated(tmp_path):
    storage_root = tmp_path / "storage"
...
def test_build_archive_returns_stored_bytes_after_seal(tmp_path):
    ...
    bytes1 = build_ree_archive(storage_root, ree_id)
    bytes2 = build_ree_archive(storage_root, ree_id)
...
def test_get_ree_document_includes_live_consistency_report(tmp_path):
    ...
    fresh = get_ree_document(storage_root, ree_id).consistency.model_dump()
    assert next(s for s in fresh["steps"] if s["step"] == "build_runtime")["status"] == "fresh"

    script.write_text("make other", encoding="utf-8")
    stale = get_ree_document(storage_root, ree_id).consistency.model_dump()
