...
usage() {
    echo "usage: ${0##*/} --suite NAME --compute-locations N --images local|published" >&2
    echo "       [--repository REPOSITORY --tag TAG]" >&2
    exit 2
}

suite=
compute_location_count=
images=local
image_repository=
image_tag=
while (($#)); do
    case "$1" in
        ...
        --compute-locations) (($# >= 2)) || usage; compute_location_count=$2; shift 2 ;;
...
[[ -n $suite ]] || usage
[[ $compute_location_count =~ ^[1-9][0-9]*$ ]] || {
    echo "compute locations must be a positive integer" >&2
    exit 2
}
case "$images" in
    local)
...
up_args=(--compute-locations "$compute_location_count")
...
if [[ $suite == demo-api ]]; then
    api/tests/e2e/api_walkthrough.py --base-url "$("$stack" api-url)"
else
    cd gui
