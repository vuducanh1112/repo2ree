...
PACKAGE_PATTERN: Final = re.compile(r'^[a-zA-Z0-9_\-]+')
EXTRAS_PATTERN: Final = re.compile(r'\[([a-zA-Z0-9_,\-]+)\]')
VERSION_SPECIFIER_PATTERN: Final = re.compile(r'[=<>!~]{1,2}.*')
...
class PythonDependency(BaseModel):
    ...
    source_type: "PythonDependencySourceType"

class PythonDependencySourceType(Enum):
    CONDA = "conda"
    PYPI = "pypi"
...
def pin_python_dependency_in_string(dependency_str: str, cutoff_date: datetime) -> str:
    ...
    python_dependency = parse_dependency_string(dependency_str)
    if not python_dependency:
        # Could not parse the dependency string, return as is
        return dependency_str
    
    if python_dependency.version_specifier:
    ...
    if version:
        pinned_python_dependency = python_dependency.model_copy(
            update={"version_specifier": f"=={version}"}
        )
        pinned_dependency_str = get_pinned_string(pinned_python_dependency)
        return pinned_dependency_str
    else:
        raise ValueError(f"Could not find a version for package '{python_dependency.name}' on PyPI before {cutoff_date.isoformat()}")
...
def get_pypi_package_info(package_name: str) -> PYPIPackageInfo:
    ...
    pypi_package_info = PYPIPackageInfo(
        name = package_name,
        releases = releases
    )
...
def parse_dependency_string(dependency_str: str) -> PythonDependency | None:
    """
    Parses a dependency string from requirements.txt or pyproject.toml
    and returns a PythonDependency object.
    ...
    package_match = PACKAGE_PATTERN.match(dependency_str)
    if not package_match:
        return None
    
    name = package_match.group(0)

    extras_match = EXTRAS_PATTERN.search(dependency_str)
    extras = extras_match.group(1).split(',') if extras_match else None

    version_specifier_match = VERSION_SPECIFIER_PATTERN.search(dependency_str)
    version_specifier = version_specifier_match.group(0) if version_specifier_match else None

    return PythonDependency(
        name=name,
        version_specifier=version_specifier,
        extras=extras,
        source_type=PythonDependencySourceType.PYPI
    )


def get_pinned_string(dependency: PythonDependency) -> str:
    """
    Constructs a pinned dependency string from a PythonDependency object and a version.
    ...
    extras_str = f"[{','.join(dependency.extras)}]" if dependency.extras else ""
    pinned_string = f"{dependency.name}{extras_str}{dependency.version_specifier}"
    return pinned_string