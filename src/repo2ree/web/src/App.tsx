...
const LEVELS: Level[] = [
  { n: 0, label: "None", color: "#94a3b8", bg: "#f1f5f9", ink: "#475569", short: "NONE", desc: "No reproducibility metadata at all.", problem: "No descriptions available.", fix: "Start by writing a README." },
...
const EVALUATE_SVC: Service = {
  ...
  requires: [
    { field: "_sourceAvailable", label: "Source loaded in workspace" },
  ],
  params: [
...
const FIELD_META: Record<string, FieldMeta> = {
  ...
  detected_dependencies: {
    ...
    howTo: "Run Evaluate after loading source. The scanner inspects requirements.txt, pyproject.toml, environment.yml, and package.json when present.",
    tools: [],
  },
  repro_level: {
    label: "Repro Level",
    ...
    format: "Auto-generated by Evaluate after a successful run.",
    howTo: "Run Evaluate to compute this value from available repository metadata and detected dependency/runtime signals.",
    tools: [],
  },
  runtime: {
    label: "Runtime",
  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeftWidth: 3, borderLeftStyle: "solid", borderLeftColor: active ? C.accent : "transparent",
        boxShadow: active ? `inset 0 0 0 1px ${C.accentBorder}` : "none",
      }}
    >
            ...
            <div key={group.path} style={{ border: `1.5px solid ${ecoMeta.color}35`, borderRadius: 10, overflow: "hidden", background: "rgba(255,255,255,0.7)" }}>
              ...
              <button onClick={() => toggle(group.path)}
                style={{
                  ...
                  borderBottomWidth: isOpen ? 1 : 0, borderBottomStyle: "solid", borderBottomColor: isOpen ? `${ecoMeta.color}25` : "transparent",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, overflow: "hidden", animation: "fadeUp 0.2s ease" }}>
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "24px 28px", display: "flex", flexDirection: "column", gap: 0 }}>
          ...
          {svc.key !== "sbom" && svc.key !== "activation" && (
            <>
              <ServiceActionSection
                ...
                idleLabel="Run"
                runningLabel="Running…"
                helperText={sourceLoadedInWorkspace
              ...
              <div
                style={{
                  background: C.surface,
                  borderTopWidth: 1, borderTopStyle: "solid", borderTopColor: hasScoreOutput ? `${svc.color}55` : C.border,
                  borderRightWidth: 1, borderRightStyle: "solid", borderRightColor: hasScoreOutput ? `${svc.color}55` : C.border,
                  borderBottomWidth: 1, borderBottomStyle: "solid", borderBottomColor: hasScoreOutput ? `${svc.color}55` : C.border,
                  borderRadius: 12,
                  padding: "16px 18px",
                  marginBottom: 20,
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: hasScoreOutput ? currentLevel.ink : C.textMuted, marginTop: 2, fontWeight: hasScoreOutput ? 600 : 500 }}>
                      {hasScoreOutput ? `Computed from latest completed Evaluate run · Standing level ${standing}` : "No Evaluate output yet. Complete a run to generate the level."}
                    </div>
                  </div>
                  <LevelBadge level={level} />
                </div>

                <div style={{ height: 7, borderRadius: 99, background: C.surfaceAlt, border: `1px solid ${C.border}`, overflow: "hidden", marginBottom: 12 }}>
                  <div
                    style={{
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 12, fontWeight: active ? 700 : 600, color: reached ? C.text : C.textMid }}>{lv.label}</div>
                          <div style={{ fontSize: 11, color: C.textMuted, lineHeight: 1.4 }}>{lv.desc}</div>
                          <div style={{ marginTop: 5, display: "flex", flexDirection: "column", gap: 4 }}>
                            <div style={{ display: "flex", alignItems: "flex-start", gap: 6, padding: "4px 6px", borderRadius: 6, background: "#fffbeb", border: "1px solid #fde68a" }}>
                              <span style={{ display: "flex", color: "#b45309", flexShrink: 0, marginTop: 1 }}>{Ic.info(11)}</span>
                              <span style={{ fontSize: 11, color: "#92400e", lineHeight: 1.35 }}>
                                {lv.problem || "No major bottleneck called out at this level."}
                              </span>
                            </div>
                            <div style={{ display: "flex", alignItems: "flex-start", gap: 6, padding: "4px 6px", borderRadius: 6, background: "#f0fdf4", border: "1px solid #bbf7d0" }}>
                              <span style={{ display: "flex", color: "#15803d", flexShrink: 0, marginTop: 1 }}>{Ic.check(11)}</span>
              ...
              <div
                style={{
                  ...
                  borderRightWidth: 1, borderRightStyle: "solid", borderRightColor: C.border,
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  ...
                  {!hasRun && <span style={{ fontSize: 11, color: C.textMuted, fontStyle: "italic", marginLeft: "auto" }}>run to scan</span>}
                </div>
                      ...
                      const scan = (nodes) => {
                        for (const n of nodes || []) {
                          if (n.type === "folder") scan(n.children);
                            ...
                            if (lo === "dockerfile" || lo === "containerfile" || lo.startsWith("dockerfile.") || lo.startsWith("containerfile.") || lo === "docker-compose.yml" || lo === "docker-compose.yaml") containerCount += 1;
