...
from repo2ree.core.python_packages_util.pin_pypi_package_version import (
    pin_package_versions_in_requirements_txt,
    pin_package_versions_in_pyproject_toml,
)
from repo2ree.core.python_packages_util.pin_conda_package_version import (
...
@click.option(
    ...
    help="Path to the requirements.txt file to be processed. Defaults to ./requirements.txt",
)
@click.option(
    "--cutoff_date",
...
@click.option(
    ...
    help="Can be a directory or a file path. If a directory, the pinned requirements file will be saved in that directory with the name <original_name>_pinned.txt. If a file path, the pinned requirements file will be saved at that exact location. Defaults parent directory of the input requirements file.",
)
def pin_requirements_txt(requirements_file: str, cutoff_date: str, output_path: str):
    ...
    if not _requirements_file.exists():
        print(
            f"Error: The specified requirements file does not exist: {_requirements_file}"
        )
        return
    ...
    if _ouput_path.is_dir():
        pinned_requirements_file_path = (
            _ouput_path / f"{_requirements_file.stem}_pinned{_requirements_file.suffix}"
        )
    else:
        pinned_requirements_file_path = _ouput_path

    requirements_file_content = _requirements_file.read_text()
    pinned_requirements_file_content = pin_package_versions_in_requirements_txt(
        requirements_file_content, _cutoff_date
    )
...
@click.option(
    ...
    help="Path to the pyproject.toml file to be processed. Defaults to ./pyproject.toml",
)
@click.option(
    "--cutoff_date",
    help="Cutoff date in YYYY-MM-DD format. If none is provided, defaults to today.",
...
def pin_pyproject_toml(pyproject_file: str, cutoff_date: str, output_path: str):
    ...
    pinned_pyproject_file_content = pin_package_versions_in_pyproject_toml(
        pyproject_file_content, _cutoff_date
    )
...
@click.option(
    ...
    help="Path to the conda environment.yml file to be processed. Defaults to ./environment.yml",
)
@click.option(
    "--cutoff_date",
    help="Cutoff date in YYYY-MM-DD format. If none is provided, defaults to today.",
...
def pin_environment_yml(environment_yml_file: str, cutoff_date: str, output_path: str):
    ...
    if not _environment_yml_file.exists():
        print(
            f"Error: The specified environment.yml file does not exist: {_environment_yml_file}"
        )
        return
    ...
    if not output_path:
        ...
        _ouput_path = Path(output_path)
    if _ouput_path.is_dir():
        pinned_environment_yml_file_path = (
            _ouput_path
            / f"{_environment_yml_file.stem}_pinned{_environment_yml_file.suffix}"
        )
    else:
        pinned_environment_yml_file_path = _ouput_path

    environment_yml_file_content = _environment_yml_file.read_text()
    pinned_environment_yml_file_content = pin_package_versions_in_environment_yml(
        environment_yml_file_content, _cutoff_date
    )
