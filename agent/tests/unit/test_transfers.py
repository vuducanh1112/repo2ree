...
def test_deliver_reassembles_chunks_in_order_and_cleans_up() -> None:
    store = TransferStore()
    transfer_id = store.open("wb", "/ree/dest.bin")
    store.write(transfer_id, b"hello ")
    store.write(transfer_id, b"world")
...
def test_abort_and_abort_all_discard_partial_files() -> None:
    ...
    store.abort(aborted)
    with pytest.raises(KeyError):
        store.write(aborted, b"more")

    store.abort_all()
    with pytest.raises(KeyError):
        store.write(dangling, b"more")
