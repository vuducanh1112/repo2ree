...
import type { StepCommand } from "./stepCommands";

type Updater<T> = T | ((previous: T) => T);

type SourceResetCommand = Extract<SourceCommand, { type: "resetStepsAfterSourceChange" }>;
...
export type WorkspaceStateCommand =
  ...
  | {
      ...
      stepParams: SourceResetCommand["stepParams"];
    }
  | {
...
export function mapStepCommandsToEffects(commands: StepCommand[]): StepEffect[] {
...
export function mapSourceCommandsToEffects(commands: SourceCommand[]): StepEffect[] {
  return commands.flatMap((command) => {
      ...
      return [
        {
          ...
          command: {
            type: "setStepRunLoading" as const,
            key: "source",
          },
        },
      ];
      ...
      return [
        {
          type: "dispatchStateCommand" as const,
          command: {
            type: "resetStepsAfterSourceChange" as const,
    ...
    if (command.type === "hydrateWorkspace") {
      const hydrateEffects: StepEffect[] = [];
      if (command.reeSpec) {
        const reeSpec = command.reeSpec;
        hydrateEffects.push({
          type: "dispatchStateCommand",
