...
import { mapSourceCommandsToEffects, mapStepCommandsToEffects } from "./stepCommandEffects";

describe("mapStepCommandsToEffects", () => {
  it("maps persist commands into shell persistence effects", () => {
    expect(
      mapStepCommandsToEffects([{ type: "persistFile", path: "sbom.json", content: "{}" }]),
  ...
  it("drops hydrateWorkspace commands with no state updates (files come from React Query)", () => {
    const effects = mapStepCommandsToEffects([
      {
