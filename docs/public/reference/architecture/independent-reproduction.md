...
![A published REE being loaded into a reviewer's workbench and reproduced through source, build, activation, and experiment stages](assets/independent-reproduction.svg)

## Reproduction sequence

1. The reviewer creates an REE on a chosen location, and its provider
   provisions one workbench.
...
6. Attempt verdicts, logs, and operation results return over the workbench
   connection and settle the corresponding runs.
...
namespaces within its durable tree rather than by provisioning a fresh
