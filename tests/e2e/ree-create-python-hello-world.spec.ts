...
test("upload source archive into workspace", async ({ page }) => {
	...
	await test.step("Upload source archive", async () => {
		await clickDemo(page, page.getByRole("button", { name: "Upload tarball" }), "Upload tarball source");
		await page
			.locator('input[type="file"][accept=".zip,.tar,.gz,.tgz,.tar.gz"]')
			.setInputFiles(sourceArchive);
		await clickDemo(page, page.getByRole("button", { name: /Add to workspace/i }), "Add source to workspace");
	});
	...
	await test.step("Seal and download", async () => {
		await clickDemo(page, page.getByRole("button", { name: /Seal.*Seal the REE/ }), "Seal the REE package");
